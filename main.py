...
class Ui_Register(object):
    ...
    def retranslateUi(self, Register):
        ...
        self.label_6.setText(_translate("Register", "Resultado de la operación"))
    ...
    def openPopUp(self):
        responseOpenFileName = QtWidgets.QFileDialog.getOpenFileName(Register, 'Open a file', '','only text format (*.txt)')
        self.path = responseOpenFileName[0]
        if self.path != ('', ''):
            self.mensajeCodificado = open( self.path ).read()
            print('El mensaje codificado es: ', self.mensajeCodificado)
            self.lineEdit.setText(self.path)
            
            self.textEdit_2.setText(self.mensajeCodificado)
            
            self.mensajeDecodificado = crearMensajeDecodificado(self.mensajeCodificado)
            
            self.textEdit.setText(self.mensajeDecodificado['mensaje'])
            self.lineEdit_2.setText(self.mensajeDecodificado['operacion'])
            self.lineEdit_3.setText(self.mensajeDecodificado['posicionInicial'])
    ...
    def comportamientoBotonCodificar(
        self,
        path,
        lineEdit_2,
        lineEdit_3,
